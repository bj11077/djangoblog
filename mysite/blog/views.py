...
def post_new(request):
    ...
    #를 출력한다. 따라서 POST 방식인지를 구분부터 해 준다.
    if request.method == "POST":
        # form변수에 빈 칸이 채워진 Form 양식을 받아옴
        form = PostForm(request.POST)
        # .is vaild()는 form에서 요청한 자료가 모두 기입되었는지
        # 체크하는 메서드, 만약 모두 기입되지 않았다면 False
        # 모두 기입되었다면 True를 반환한다.
        if form.is_valid() :
            #form.save()는 아무것도 입력을 안 하면 DB에 바로
            # 데이터를 적재해버리기 때문에 우선 form 양식이외에
    ...
    # 폼을 자동으로 구성할 예정임.
        ...
        #  그냥다시 post방식 작성을 유도하기 위해 폼으로
        # 이동시키는 로직 실행
        form = PostForm()  #forms.py에서만든거임
...
def post_edit(request, pk):
    ...
    # 1번로직이라면 기존 글 정보를 받아오기 위해 get_object_or_404를
    # 2번로직이라면 폼에서 받아온 정보를 기존자료에 덮어씌우기 위해서
